...
class CentrifugeController:
    ...
    reconnect = True
    ## Missing _cycle_running variable

    def __init__(self):
        self._cycle_running = False
        ## Constructor missing initialization of the other member variables
    ...
    def disconnect(self):
        self.port.close()
        if self.reconnect:
            self.connect() ## missing parameter 
    ...
    def get_speed_in_thread(self):
        ...
        if result == b"VIBRTION": ## Vibration is spelled wrong, or VIBRITION is used to define a constant, which is a poor design choice 
    ...
    def perform_centrifuge_cycle(self, name, cycle):
        ...
        for step in cycle.split("\n"):
            ...
            self.target_speed = s
            while not self.got_speed > self.target_speed: ## Could use while self.got_speed < self.target_speed, poor design choice 
                self.getSpeed()
    ...
    def find_max_speed_before_vibration(self):
        ...
        ## Above commented code is the same as the code below 

        speed = 10 ## variable for speed same as function for speed, causes confusion
        self._vibration_callback = self.vib_callback
        while speed != self._speed_cap:
            # Set the speed
            self.speed(speed) 
            ...
            while (datetime.now() - test_start).total_seconds() < 10:
                ...
                except: ## No condition given for the excpetion 
                    pass
                if self.did_vibrate:
                    return speed
            speed = speed + 100 ## speed change is too big, should be set to smaller intervals 
    ...
    def average_speed(self):
        ...
        for e in self._speeds:
            accum = accum + e[1] ## e[1] wouldn't cycle through the list _speeds, would only cycle through the first term of the list 
    ...
    def speed_standard_dev(self):
        ...
        #     last_speed = e[1]
        # return deviation  #all this code is commented only to be repeated later
        ## Above code is commented out only to be repeated later, with no valid reason given 
        ...
        deviation = 0
        for e in self._speeds:
            deviation += e[1] - average ## standard deviation is found incorrectly 
        return deviation

    def max_speed(self):
        max_speed = 0
        for e in self._speeds:
            max_speed = max(max_speed, e[1]) ## e[1] wouldn't cycle through the list, so the max speed would only be compared to one value
        return max_speed
    ...
    def save_log(self):
        ...
        log_f.write(b'SC8.1')
        for e in self._speeds:
            log_f.write(struct.pack("<HH", int(calendar.timegm(e[0].utctimetuple())), e[1])) ## This code overwrites everything that was saved in the preivous calendar, instead of individually finding where a log needs to be saved/replaced
